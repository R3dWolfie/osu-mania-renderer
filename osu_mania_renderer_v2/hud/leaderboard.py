"""MAP RENDER LEADERBOARD — the per-map board on the osu!mania results screen.

VERBATIM PORT of the osu!catch renderer's hud/leaderboard.py (itself a
...
Discord-avatar path) so mania reaches parity with std/catch. The ONLY
mania-specific changes vs the catch module (each marked MANIA below):
  * the avatar cache dir (r3d-mania) + HTTP User-Agent;
  * count_geki (the 320/rainbow) is pulled ALONGSIDE catch's count_katu (which
    mania reads as the 200) so a flank card can show mania's six judgment
    tiers; a render DB predating those columns degrades gracefully — the
    query retries with std's exact column set and the 320/200 rows read 0.

The current play's results (the ported lazer featured panel,
hud/lazer_results.py) is flanked by compact ranked cards of the OTHER renders
of the SAME map, pulled from the R3D render DB (LOCAL-ONLY — no osu!API, same
rule as std's board).

This module is the DATA + AVATAR layer (pure); the card BAKING + compositing
live in lb_cards.py, reusing mania's results font + judgment palette so the
flanking cards match the featured panel.
...
# best-per-player columns pulled for each flanking card. MANIA: count_geki
# (the 320/rainbow) + count_katu (mania's 200) extend std's set — catch
# hud/leaderboard.py:37-41 pulled count_katu the same way for its droplets.
_LB_COLUMNS = (
    "player_name", "discord_user_id", "score", "accuracy", "grade",
    "max_combo", "mods_str", "mods", "count_300", "count_100", "count_50",
    "count_miss", "count_geki", "count_katu", "replay_md5",
)

# std's exact column set (osu_std_renderer/render/leaderboard.py:28-33) — the
# schema-drift fallback: a render DB without the mania judgment columns still
# yields a board, with the 320/200 rows reading 0.
_LB_COLUMNS_BASE = (
    "player_name", "discord_user_id", "score", "accuracy", "grade",
    "max_combo", "mods_str", "mods", "count_300", "count_100", "count_50",
...
class LeaderboardEntry:
    ...
    count_geki: int = 0                     # MANIA: 320 (rainbow) tally
    count_katu: int = 0                     # MANIA: 200 tally
...
def query_leaderboard(db_path, beatmap_md5: str,
    ...
    try:
        ...
        # MANIA: two passes — the full column set first, then std's base set
        # when the DB predates count_geki/count_katu (schema drift ≠ no board).
        for cols in (_LB_COLUMNS, _LB_COLUMNS_BASE):
            ...
            try:
                ...
                break
            except Exception:  # noqa: BLE001 — missing column → base set
                rows = None
...
def _entry_from_row(rank: int, row: dict) -> LeaderboardEntry:
    return LeaderboardEntry(
        ...
        count_geki=int(row.get("count_geki") or 0),
...
def rows_from_osu_json(entries: list) -> list[dict]:
    ...
    count300, count100, count50, count_miss, avatar_png (path|null)} — the
    EXACT contract catch's parser reads (catch hud/leaderboard.py:221-251),
    plus mania's count_geki/count_katu read the same way catch reads its
    count_katu (absent keys → 0). Robust to missing keys. discord_user_id is
    None (osu players aren't Discord-linked); avatar_png carries the
    pre-fetched osu avatar the flank card loads."""
    rows: list[dict] = []
    for e in entries or []:
        ...
        rows.append({
            ...
            "count_geki": int(e.get("count_geki") or 0),  # MANIA: 320s
            "count_katu": int(e.get("count_katu") or 0),  # MANIA: 200s
            "replay_md5": None,
...
AVATAR_CACHE_DIR = os.path.expanduser("~/.cache/r3d-mania/avatars")
...
def fetch_discord_avatar(discord_user_id: str, token: str,
    ...
    try:
        body = _https_get(
            ...
            {"Authorization": f"Bot {token}", "User-Agent": "r3d-mania/1.0"},
            timeout, proxy_url)
        ...
        img = _https_get(
            ...
            {"User-Agent": "r3d-mania/1.0"}, timeout, proxy_url)
        return img
