"""Compact ranked FLANK CARDS for the osu!mania results screen — the per-map
render leaderboard (owner mockup 2026-07-09), ported from the osu!catch
renderer's hud/lb_cards.py (itself the port of the osu!STANDARD renderer's
render/lazer_results.py flank cards) so mania reaches parity.

The DATA + avatar layer is leaderboard.py (query_leaderboard / build_board /
rows_from_osu_json / resolve_avatar_bytes — a verbatim port of the catch/std
module). THIS module bakes each LeaderboardEntry into a rounded card image and
lays the board out around the lazer featured panel (hud/lazer_results.py),
which composites the cards through its ported catch draw path (_draw_board —
std slide-in timing, stage-2 tracking).

Card layout/constants are catch's VERBATIM (catch hud/lb_cards.py:41-50 sizes,
:140-164 panel bake, :167-211 avatar chip, :229-327 card bake, :389-449 board
layout). Adaptations vs the catch flank cards (the same per-mode deltas catch
itself made vs std):
  * fonts   = the bundled Nunito the mania results screen uses
              (hud/lazer_results.py _nunito_loader — body weight for labels,
              score weight for the big score, exactly the loaders std passes
              its bake_lb_card, osu_std_renderer/render/lazer_results.py:1618-1620),
              NOT catch's skin font.
  * palette = mania's SIX judgment tiers (320/300/200/100/50/Miss) in the
              engine's judgment palette (lazer_results.MANIA_RESULT_COLORS)
              + lazer's OsuColour.ForRank grade colours for the grade pill,
              so the flanks read as the same UI as mania's featured panel.
              The judgment block is 6 rows instead of catch's 4 — the card's
              vertical-balance rule (catch :262-271) absorbs the difference.
"""
...
# render DB — same file + env override the std/catch renderers use (a symlink
# resolves this path to state/db.sqlite on the render box). Local-only,
# read-only. (catch hud/lb_cards.py:38-39, verbatim)
...
LB_CARD_H = 596.0                 # (catch hud/lb_cards.py:41-42, verbatim)

# Fraction of the frame width reserved DOWN THE CENTRE for the featured
# results panel; the flank cards start just outside it. Legacy fallback only —
# build_mania_board passes the lazer featured panel's width instead (catch
# hud/lb_cards.py:44-50 + :569-575).
CENTER_CLEAR_FRAC = 0.40

# mania judgment palette (0..1) — MUST match hud/lazer_results.py
# MANIA_RESULT_COLORS (the engine's own results colours: gpu/renderer.py
# jcolours + the UR-histogram bands) so the flanks read as the same UI as the
# featured panel. Defined locally (not imported) so this module stays
# PIL-only, exactly like catch's lb_cards keeps its palette local (:52-60).
RESULT_COLORS = {
...
# grade PILL colours — lazer's OsuColour.ForRank (the exact hexes
# hud/lazer_results.py:145-157 paints the featured grade letter with), so the
# flank pill matches mania's featured grade. (catch used its legacy hud grade
# palette here for the same reason — its featured card predates its lazer
# port; mania's featured screen IS the lazer port, so ForRank is the match.)
...
FOR_RANK["F"] = FOR_RANK["D"]

# the arc-gradient cyan std uses for the NEW BEST pill (its ForRank S adjacent)
_ARC_CYAN = (0x7C / 255, 0xF6 / 255, 0xFF / 255)


# --- fonts (mania's results font — the bundled Nunito, lazer-Torus stand-in) --------
# The same asset + variable-weight loaders hud/lazer_results.py builds
# (_nunito_loader / RESULTS_TEXT_WEIGHT / RESULTS_SCORE_WEIGHT), duplicated
# here so this module stays importable without the GL stack — the same reason
# catch's lb_cards resolves its own font (catch hud/lb_cards.py:34).

NUNITO_PATH = os.path.normpath(os.path.join(
    os.path.dirname(os.path.dirname(__file__)), "assets", "fonts",
    "Nunito[wght].ttf"))
_TEXT_WEIGHT = 500             # Nunito Medium (body/labels)
_SCORE_WEIGHT = 330            # Nunito Light (the big score)


def _nunito(weight: int):
    def _load(px: int):
        px = max(int(px), 6)
        try:
...
_load_font = _nunito(_TEXT_WEIGHT)      # body/labels (std's _font_loader role)
_load_score_font = _nunito(_SCORE_WEIGHT)  # big score (std's _score_loader role)


# --- small helpers (catch hud/lb_cards.py:81-135, verbatim) -------------------------
...
# --- bakers (catch hud/lb_cards.py:138-227, verbatim) -------------------------------
...
def bake_lb_card(entry, avatar_bytes, W_px: int, H_px: int, k: float,
    ...
    name, the 320/300/200/100/50/Miss rows, Max Combo, Accuracy, mods badges,
    the big score, and the grade pill. `entry` is a leaderboard.LeaderboardEntry.
    Returns a PIL RGBA image. (catch hud/lb_cards.py:229-327 — verbatim layout;
    the judgment block is mania's six tiers instead of catch's four.)"""
    W = max(int(W_px), 60)
    H = max(int(H_px), 120)
    rad = max(int(18 * k), 4)
    ...
    # instead of leaving one big empty gap above the score (catch :262-271 —
    # 6 judgment rows here instead of catch's 4, same rule).
    ms = (entry.mods_str or "").strip()
    has_mods = bool(ms and ms.upper() != "NM")
    block_h = 6 * int(23 * k) + int(8 * k) + int(23 * k) + int(30 * k)
    if has_mods:
        block_h += int(22 * k)
    slack = (H - int(84 * k)) - y - block_h
    y += max(0, int(slack * 0.5))
    # mania judgment rows — the six tiers the featured panel's grids show
    # (320|300|200 then 100|50|MISS), one per row in the same palette.
    for lbl, val, col in (("320", entry.count_geki, RESULT_COLORS["320"]),
                          ("300", entry.counts[0], RESULT_COLORS["300"]),
                          ("200", entry.count_katu, RESULT_COLORS["200"]),
                          ("100", entry.counts[1], RESULT_COLORS["100"]),
                          ("50", entry.counts[2], RESULT_COLORS["50"]),
...
# --- board assembly + layout (once per render) — catch :351-476, verbatim -----------
...
class BakedBoard:
    ...
    compact: bool = False                # a side has 2+ cards (catch's old
                                         # text-stack hint; the lazer screen
...
def bake_board(board, W: int, H: int, title: str, *,
    ...
    render / empty board) — the caller then draws the plain results screen,
    UNCHANGED.
    ...
    `center_clear_px` overrides the cleared centre width (build_mania_board
    passes the lazer featured panel's width so the cards hug the panel edges
    exactly like the std/catch results screens); None → the legacy text-stack
    ...
    for i, entry in enumerate(reversed(left)):
        img = bake_lb_card(entry, _avatar_bytes_for(entry, resolve_avatar_fn),
                           cw, ch, scale, _load_font, _load_score_font)
...
def _bake_banner(board, title, scale, W, H):
    """'#RANK on <map>' + the NEW #1 / NEW BEST pill. The lazer screen's
    _draw_board anchors the banner just above the featured panel itself
    (hud/lazer_results.py — catch's ported placement), so only the baked image
    matters; the (cx, cy) here is the legacy top-edge rest position."""
...
# --- orchestration (called once from render.py / compositor.py) ---------------------

def build_mania_board(cfg, replay, bm, replay_md5: str):
    """Build + bake the mania results-screen leaderboard for one render, or
    None. (catch hud/lb_cards.py:521-586 build_catch_board, verbatim shape.)
    ...
    `replay` = the parsed ReplayInfo (.osr header truth: beatmap_md5,
    player_name, score); `bm` = the BeatmapInfo (banner title)."""
    from osu_mania_renderer_v2.hud import leaderboard as lb
    ...
    if src == "osu" and lb_json:
        try:
            ...
            used_osu = False
    if not rows:
        rows = lb.query_leaderboard(DB_PATH,
                                    getattr(replay, "beatmap_md5", "") or "",
                                    replay_md5)

    cur_score = int(getattr(replay, "score", 0) or 0)
    ...
    cur_name = (getattr(replay, "player_name", "") or "").strip().lower()
    prev_best = None
    for r in rows:
    ...
    board = lb.build_board(rows, getattr(replay, "player_name", "") or "",
                           cur_score, prev_best_score=prev_best,
                           max_per_side=3)
    if not board.left and not board.right:
        return None

    # mania's BeatmapInfo names the diff `difficulty` (std/catch: version)
    diff = getattr(bm, "difficulty", "") or ""
    title = f"{bm.artist} - {bm.title} [{diff}]".strip(" -")
    ...
    try:
        from osu_mania_renderer_v2.hud.lazer_results import PANEL_W as _PANEL_W
