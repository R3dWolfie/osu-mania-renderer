"""Background dim envelope — the std engine's DimEnvelope ported to mania v2
...
Semantics (std §4.10, R3D preset keys bg_dim_intro/game/breaks 0-100%):
    ...
    intro dim   HELD until the FIRST note's approach begins (time_ms -
                the effective approach window); the glide INTO gameplay dim
                STARTS there and runs over GLIDE_MS, so the background is
                still bright as the first note scrolls in and dims
                underneath it (danser)
    ...
    breaks dim  during [Events] breaks: the brighten glide starts at the
                ...
                note's approach start)) — the background stays bright until
                gameplay resumes and dims during the first post-break
                approach
    breaks too short to fit both glides are skipped (dim stays at game)

Mania's breaks are plain ``(start_ms, end_ms)`` tuples
(beatmap._parse_breaks, rescaled to REAL/video time by mods.apply_mods like
every note) — the same shape catch uses, so build_dim_envelope is catch's
verbatim. `preempt` here is the render's effective approach window
(render.APPROACH_MS scaled by scroll_speed — RenderPlan.effective_approach_ms,
the on-screen travel time of a note). Levels are the fractions 0..1 that
RenderOptions.bg_dim_* already store. Pure math, no GL — the GPU renderer's
`_draw_background` samples ``level(t)`` per frame over whatever background it
draws (the static image today; the background-video frame takes the exact
same path when the load_video wiring lands, so video dims identically).
"""
